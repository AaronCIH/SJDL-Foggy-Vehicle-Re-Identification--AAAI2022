...
def make_loss_for_syn(cfg):
    ...
    if sampler == 'SJDL':
        print("############### Loss define ##############")
        print("baseline_loss_w = {%f}, rest_loss_w = {%f}"%(cfg.MODEL.SJDL_BASE_W
, cfg.MODEL.SJDL_REST_W))
        print("##########################################")
        def loss_func(score, feat, target, gx_fake, gx_enh, gt):          
            ...
            # total loss
            con_loss = (cfg.MODEL.SJDL_BASE_W
 * Lbaseline) + (cfg.MODEL.SJDL_REST_W * (Lgan_fake+Lgan_enh)/2)
...
, cfg.MODEL.SJDL_REST_W))
        ...
        def loss_func(score, feat, target, hazy, gx_ehn): 
            ...
            # total loss
            total_loss = (cfg.MODEL.SJDL_BASE_W
 * Lbaseline) + (cfg.MODEL.SJDL_REST_W * Lgan)
