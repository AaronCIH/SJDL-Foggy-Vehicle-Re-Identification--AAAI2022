...
root_path = '../';  
run( [root_path, '/libs/vlfeat-0.9.18/toolbox/vl_setup.m'] );
fprintf("vlfeat finish");

dir_matConvNet= [root_path, '/libs/matconvnet-1.0-beta24/matlab/'];
addpath(genpath(dir_matConvNet));
run([dir_matConvNet 'vl_setupnn.m']);
fprintf("matconvnet finish"); 

opts=[];

% img_dir= root_path + '/Make3D/';
% img_type='outdoor';

% img_dir= root_path + '/NYUD2/';
% img_type='indoor';
...
CLEAR_PATH = ('../../FVRID_syn/train_clear');   % please check your path !!!!
SAVE_PATH = ('../../FVRID_syn/Depth/train_clear/');  % please check your path !!!!
...
result_dir=fullfile(SAVE_PATH, img_dir_name);
...
if strcmp(img_type, 'outdoor') 
    ...
    %outdoor scene model
    trained_model_file= [root_path, '/model_trained/model_dcnf-fcsp_Make3D']; 
end
...
if strcmp(img_type, 'indoor')
    ...
    %indoor scene model
    trained_model_file= [root_path, '/model_trained/model_dcnf-fcsp_NYUD2'];   
end
...
fprintf("total_amount: %d",img_num);
if img_num==0
